...
class SequenceGeneratorWithClasses(object):
    ...
    def __init__(self, network, dictionary=None, seq_maxlen=25,
                 clip_gradients=0.0, extra_vect_len = 0, tensorboard_verbose=0,
        ...
        self.extra_vec_len = extra_vect_len
    ...
    def generate(self, seq_length, temperature=0.5, extra_class = None, seq_seed=None,
                 display=False):
        """ Generate.
        ...
        for i in range(seq_length):
            x = np.zeros((1, self.seq_maxlen, len(self.dic) + self.extra_vec_len))
            for t, char in enumerate(sequence):
                x[0, t, self.dic[char]] = 1.
                if extra_class is not None:
                    x[0, t, len(self.dic) + extra_class] = 1.
